...
@Slf4j
@Service
public class PersonalCargoOwnerFastAuthStrategy implements AuthStrategy {
    ...
    public AuthResultResponse auth(AuthBusReqDTO authBusReqDTO) {
        log.info("【个人货主快速认证】使用上上签个人2要素认证参数:{}", JsonUtil.toString(authBusReqDTO.getFillContent()));
        ...
        log.info("【个人货主快速认证】使用上上签个人2要素认证姓名:{},身份证号:{},认证结果:{}", map.get(AuthRequestConstant.REAL_NAME), map.get(AuthRequestConstant.ID_CARD_NO), JsonUtil.toString(authResultResponse));
        return authResultResponse;
    }

    @Override
    public String getType() {
        return AuthStrategyEnum.PERSONAL_CARGO_OWNER_FAST_AUTH.getCode();
    }
}
